...
df -h /nix || true # BEST_EFFORT: optional cleanup or probe failure is non-fatal.

info "Step 1/3: Deleting old user-profile generations..."
nix-collect-garbage --delete-old || true # BEST_EFFORT: optional cleanup or probe failure is non-fatal.
...
if [ -x /run/current-system/bin/switch-to-configuration ]; then
    ...
    sudo /run/current-system/bin/switch-to-configuration boot || true # BEST_EFFORT: optional cleanup or probe failure is non-fatal.
fi
