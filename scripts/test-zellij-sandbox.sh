...
cleanup() {
    if [[ -d "${CLEANUP_TARGET:-}" ]] && [[ "$CLEANUP_TARGET" == "$REAL_REPO/.sandbox"* ]]; then
        ...
        if [ -n "${TEST_SESSION:-}" ]; then
            zellij kill-session "$TEST_SESSION" 2>/dev/null || true # BEST_EFFORT: cleanup tolerates a session that already exited.
            zellij delete-session "$TEST_SESSION" --force 2>/dev/null || true # BEST_EFFORT: cleanup tolerates a session that was already removed.
        fi
...
ACTIVE_SESSIONS="$(zellij list-sessions --short --no-formatting 2>/dev/null || true)" # BEST_EFFORT: an empty isolated socket is a valid no-session result.
...
sleep 1
REMAINING="$(zellij list-sessions --short --no-formatting 2>&1 || true)" # BEST_EFFORT: listing after termination is diagnostic-only and may return no sessions.
